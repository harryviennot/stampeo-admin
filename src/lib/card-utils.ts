import { CardDesign, CustomStampConfig, RewardTier } from "@/types/design";
import { rgbToHex } from "@/lib/color-utils";

export { rgbToHex } from "@/lib/color-utils";

/**
 * Sample balance for points-card PREVIEWS.
...
// the showcase/web card-utils.ts. Change them together or the preview drifts
...
/** 'overlap' x-advance clamp, fraction of icon size. */
 ...
 *  3 levels for 17-24 ('overlap' only). */
 ...
 *  vertical padding, so rows sit closer and icons use more of the strip. */
 ...
 * calculate_staggered_layout in backend strip_generator.py.
 */
...
/**
 * Box size for a custom icon in a cell. Mirrors _paste_slot_icon in backend
 * strip_generator.py.
 */
export function customIconBoxSize(
  ...
  return Math.min(
    layout.diameter * CUSTOM_ICON_SCALE,
    layout.diameter + allowedExtra
  );
}

/**
 * Which image URL a slot renders in custom mode. Mirrors
 * _custom_icon_for_slot in backend strip_generator.py.
 */
