...
/**
 * The five strings the strip renders. web/ feeds these from next-intl
 * (`designEditor.pointsStrip`); admin has no i18n, so they default to English.
 * Hoisted to a prop rather than hardcoded so this stays a pure component and
 * a future locale-aware admin surface can override it.
 */
export interface PointsStripLabels {
  objective: string;
  nextWord: string;
  complete: string;
  /** e.g. (120) => "120 pts" */
  points: (n: number) => string;
  /** e.g. (500) => "500 pts limit" */
  limit: (max: number) => string;
}

export const DEFAULT_POINTS_STRIP_LABELS: PointsStripLabels = {
  objective: "Objective",
  nextWord: "Next",
  complete: "Card completed",
  points: (n) => `${n} pts`,
  limit: (max) => `${max} pts limit`,
};
...
interface PointsStripProps {
  ...
  /** Strip copy. Defaults to English. */
...
export function PointsStrip({
  ...
  backgroundColor,
  maxLimit,
  labels = DEFAULT_POINTS_STRIP_LABELS,
}: PointsStripProps) {
  ...
  const unit = (n: number) => labels.points(n);
      ...
      {isComplete && !nextReward ? (
        ...
        <>
          ...
          {afterReward && (
            ...
            >
              {labels.nextWord} → {unit(afterReward.threshold)}
            </span>
          )}
        </>
      )}
